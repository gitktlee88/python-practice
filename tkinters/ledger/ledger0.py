...
import ledger_bk0 as ledger_bk
...
def search_command():
    lb.delete(0,END)
    for row in ledger_bk.search(name=name.get(),\
                                user=user.get(),\
...
def add_command():
    ledger_bk.add(name.get(),user.get(),\
                  password.get(),category.get(),cdate.get())
    lb.delete(0,END)
    lb.insert(END,name.get(),user.get(),\
              password.get(),category.get(),cdate.get())
...
def update_command():
    ledger_bk.update(selected_tuple[0],name.get(),\
                     user.get(),password.get(),\
                     category.get(),cdate.get())
    view_command()
